...
func TestFilesystemRead(t *testing.T) {
	...
	// Create a Filesystem whose sandbox uses the mock HTTPClient.
	fs := &Filesystem{sandbox: sb}

	// Filesystem.Read builds a URL via sb.DownloadURL whose domain does not
	// match the test server, so the HTTPClient transport must route the
	// request to the test server.
	sb.client.config.HTTPClient = httpServer.Client()
	// Use a custom transport that redirects all HTTP requests to the test server.
...
func TestFilesystemWrite(t *testing.T) {
	...
	// The mux handles HTTP uploads and the Stat RPC.
	mux := http.NewServeMux()
	...
	// ConnectRPC Stat endpoint (Write calls GetInfo on success).
...
func TestFilesystemWriteFiles(t *testing.T) {
	...
	// Multiple files use a single batch upload.
	if uploadCount != 1 {
		t.Errorf("upload count = %d, want 1", uploadCount)
	}
	// GetInfo is called for each file.
	if statCalls != 3 {
...
func TestCommandsList(t *testing.T) {
	...
	// The second process has no config.
	if infos[1].PID != 2 {
...
// rewriteTransport redirects all HTTP requests to the given baseURL (test server).
...
func (t *rewriteTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	// Preserve the original path and query; replace scheme + host.
	orig := req.URL.String()
	_ = orig
	req.URL.Scheme = "http"
...
// TestKeepaliveHeaderInjected verifies that PTY sessions created via Sandbox.Pty() carry the Keepalive-Ping-Interval header.
func TestKeepaliveHeaderInjected(t *testing.T) {
	...
	// Exercise the full Sandbox.Pty() path so processClient() invokes keepaliveInterceptor.
	domain := "test.dev"
	c := &Client{config: &Config{
...
// TestKeepaliveHeaderInjectedCommands verifies that command sessions created via Sandbox.Commands().Start() carry the Keepalive-Ping-Interval header.
func TestKeepaliveHeaderInjectedCommands(t *testing.T) {
	...
	// Exercise the full Sandbox.Commands() path so processClient() invokes keepaliveInterceptor.
	domain := "test.dev"
	c := &Client{config: &Config{
		HTTPClient: &http.Client{
			Transport: &rewriteTransport{base: http.DefaultTransport, baseURL: ts.URL},
